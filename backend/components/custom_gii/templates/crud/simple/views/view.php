        ...
        foreach ($generator->getTableSchema()->columns as $column)
        {
            ...
            else
            {
                ...
                elseif($column->name === 'photo' || $column->name === 'image')
                {
                    ...
                    [
                    'attribute'=> '<?= $name ?>',
                    'value'=> (isset($model-><?= $name ?>) && !empty($model-><?= $name ?>))? GlobalFunctions::getFileUrlByNamePath('<?= $lower_current_class ?>',$model-><?= $name ?>) : GlobalFunctions::getNoImageDefaultUrl(),
                    'format' => ['image',['width'=>'175','height'=>'150', 'class' => 'modalImage']],
                    ],
                    <?php
                    echo "\n";
                }
...
<?php if($add_modal) {
    echo "<?= GlobalFunctions::showModalHtmlContent(Yii::t('backend','Imagen')) ?>";
}
?>
